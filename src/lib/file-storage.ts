...
const DEFAULT_CLEANUP_INTERVAL = process.env.CLEANUP_INTERVAL
  ? Number(process.env.CLEANUP_INTERVAL)
  : 24 * 3600000; // 默认每24小时清理一次 (24小时 = 24 * 3600000毫秒)
...
export function getCleanupSchedulerStatus(): {
  ...
  return {
    ...
    intervalMs: cleanupInterval ? DEFAULT_CLEANUP_INTERVAL : null
  };
}

// 预览内容存储
export async function savePreviewContent(
  previewId: string,
  html: string,
  ...
  const metadataPath = path.join(PREVIEWS_DIR, `${previewId}.meta.json`);
...
export async function getPreviewContent(
  previewId: string
): Promise<{ html: string; expired: boolean }> {
  const previewPath = path.join(PREVIEWS_DIR, `${previewId}.html`);
  const metadataPath = path.join(PREVIEWS_DIR, `${previewId}.meta.json`);
...
// 临时图片存储
export async function saveTemporaryImage(
  imageId: string,
  content: Buffer,
  expireSeconds: number = 7200
): Promise<string> {
  ...
  // 确定扩展名
  const imageExt = determineImageExtension(content);
  const imagePath = path.join(IMAGES_DIR, `${imageId}.${imageExt}`);
  const metadataPath = path.join(IMAGES_DIR, `${imageId}.meta.json`);
...
// 读取临时图片
export async function getTemporaryImage(
  imageId: string
): Promise<{ content: Buffer | null; contentType: string; expired: boolean }> {
  // 确保目录存在
  ensureDirectoryExists(IMAGES_DIR);
  ...
  try {
    ...
    const contentType = getContentTypeFromExtension(metadata.extension);
...
export async function cleanupExpiredFiles(): Promise<void> {
  try {
    Promise.all([cleanupDirectory(PREVIEWS_DIR), cleanupDirectory(IMAGES_DIR)]);
  } catch (error) {
...
// 辅助函数：根据图片内容确定文件扩展名
function determineImageExtension(buffer: Buffer): string {
  // 检查文件签名（魔数）
